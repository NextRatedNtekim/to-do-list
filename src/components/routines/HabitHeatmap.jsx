...
/**
 * src/components/routines/HabitHeatmap.jsx
 * Fixed: CSS vars only, Lucide icons, theme-aware
 */
import { useMemo } from 'react'
import { motion } from 'framer-motion'
import { startOfWeek, addDays, subWeeks, format, isToday } from 'date-fns'

const WEEKS      = 13
const WEEK_START = { weekStartsOn: 1 }

// Intensity levels using CSS vars
function intensityStyle(count) {
  if (count === 0) return { background: 'var(--bg-surface-3)', boxShadow: 'none' }
  if (count === 1) return { background: 'rgba(34,197,94,0.28)', boxShadow: 'none' }
  if (count === 2) return { background: 'rgba(34,197,94,0.58)', boxShadow: '0 0 4px rgba(34,197,94,0.3)' }
  return               { background: 'var(--brand)',            boxShadow: '0 0 8px var(--brand-glow)' }
}
...
export function HabitHeatmap({ completedDates = [], title = 'Activity' }) {
  ...
  const monthLabels = useMemo(() => {
    ...
    grid.forEach((week, wi) => {
      const m = week[0].date.getMonth()
      if (m !== lastMonth) { labels.push({ wi, label: format(week[0].date, 'MMM') }); lastMonth = m }
    })
    return labels
  }, [grid])
    ...
    <div className="card-sm" style={{ padding: '14px 16px' }}>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        ...
        <span style={{ fontSize:11,color:'var(--text-muted)',fontFamily:'var(--font-body)' }}>
          {completedDates.length} completions
        </span>
      </div>
        ...
        {grid.map((_, wi) => {
          ...
          return (
            <div key={wi} style={{ width:14,flexShrink:0 }}>
              {lbl && <span style={{ fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-body)' }}>{lbl.label}</span>}
            </div>
          )
        })}
      </div>
      ...
      {/* Grid */}
      <div style={{ display:'flex',gap:3,overflowX:'auto' }} className="scrollbar-none">
        {grid.map((week, wi) => (
          <div key={wi} style={{ display:'flex',flexDirection:'column',gap:3 }}>
            {week.map((cell, di) => {
              const { background, boxShadow } = intensityStyle(cell.count)
              return (
                <motion.div
                  key={cell.key}
                  title={`${cell.key}: ${cell.count} completion${cell.count !== 1 ? 's' : ''}`}
                  ...
                  style={{
                    ...
                    outline:       isToday(cell.date) ? '2px solid var(--brand)' : 'none',
                    outlineOffset: '1px',
                    cursor:        'default',
                  }}
                />
      ...
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:10 }}>
        ...
        <div style={{ display:'flex',gap:3 }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width:11,height:11,borderRadius:2,...intensityStyle(i) }} />
          ))}
        </div>
