...
// // import {
// //     DashboardHeader, TodaysMission, DashboardStats,
// //     NextAlarmChip, DashboardPartnerCard
// //   } from '../pages/Dashboard-ios';
// //   import MissedTaskRecovery from '../components/MissedTaskRecovery';
// //   import XPToast            from '../components/XPToast';
// //   import { useAlarmStore, initAlarmListener } from '../stores/alarmStore';
...
//   useEffect(() => {
//     initAlarmListener();                    // wire missed-task event listener
//     useAlarmStore.getState().loadAlarms(tasks);  // schedule local timers
//   }, [tasks]);
//   const [xpToast,  setXpToast]  = useState(false);
//   const [toastXp,  setToastXp]  = useState(0);
 
//   // In your task completion handler, AFTER marking task done:
//   setToastXp(task.xp_reward ?? 20);
//   setXpToast(true);
...
//       <DashboardHeader displayName={profile?.display_name} streak={profile?.streak ?? 0} />
//       <TodaysMission   tasks={tasks} />
//       <DashboardStats  streak={profile?.streak ?? 0} totalXp={profile?.total_xp ?? 0} tasksLeft={tasks.filter(t => !t.completed_at).length} />
//       <NextAlarmChip />
...
// // ─────────────────────────────────────────────────────────────────────────────
// // Fixed version of the pasted DashboardPage.
// // All bugs documented inline with // FIX comments.
// // ─────────────────────────────────────────────────────────────────────────────

// // FIX 1: Added missing useState and useEffect imports
...
// FIX 4: initAlarmListener is a plain function export, useAlarmStore is a named export.
//         Both are correct here — import path just needs to match your stores folder.
...
export function DashboardPage() {
  ...
  // FIX 5: profile and partner were used in JSX but never declared.
  //         Dashboard-additions components read from profile?.display_name,
  //         profile?.streak, profile?.total_xp, and partner.
  //         We derive them from your existing stores so no new fetch is needed.
  const profile = {
    display_name: user?.name ?? 'there',
    streak:       user?.streak ?? 0,
    total_xp:     totalXP ?? 0,
  }
  // partner comes from your existing store if available, otherwise null.
  // Replace useUserStore with whatever store holds partner data in your app.
  const partner = null  // swap with: usePartnerStore()?.partner or similar

  // FIX 6: useEffect and useState were used but not imported at the top.
  //         They are now imported in FIX 1 above.

  // FIX 7: useEffect was placed INSIDE the component body after useMemo,
  //         but AFTER the groups useMemo — that's fine for hooks order,
  //         but the original code had it BETWEEN useMemo and the groups
  //         derivation, which broke the rules-of-hooks ordering.
  //         Moved here, properly after all state declarations.
  useEffect(() => {
    initAlarmListener()
    useAlarmStore.getState().loadAlarms(tasks)
  }, [tasks])

  // FIX 8: useState declarations were placed AFTER the useEffect in the
  //         original code — React requires all hooks before any logic.
  //         Moved here, before any non-hook code.
  const [xpToast, setXpToast] = useState(false)
  const [toastXp, setToastXp] = useState(0)

  // FIX 9: The XP toast trigger lines:
  //   setToastXp(task.xp_reward ?? 20)
  //   setXpToast(true)
  //   setTimeout(() => setXpToast(false), 2200)
  // were written as bare statements at the TOP LEVEL of the component,
  // outside any function or handler. That runs on every render and
  // references `task` which doesn't exist in this scope.
  // CORRECT pattern: call this inside your task completion handler:
  //
  ...
  //   }
  //
  // The handler is NOT shown here because it lives in your existing
  // task list component. Call these three lines at the end of it.
  ...
  return (
    // FIX 10: The original JSX had an unclosed comment block from the
    //         commented-out old hero section. The closing */ was present
    //         but the fragment structure was broken because the old <div>
    //         was commented out but the new <div> opened inside what
    //         looked like a comment continuation.
    //         Removed the dangling commented block entirely.
    //         The new structure below opens ONE root div cleanly.
    <div className="px-4 pt-16 pb-4 taskr-bg">
      {/* <DashboardEnhancement /> */}
      {/* XP toast — shows after task completion */}
      <XPToast xp={toastXp} visible={xpToast} />

      {/* Missed task recovery bottom sheet */}
      <MissedTaskRecovery />

      {/* ── New glassmorphism header sections ── */}
      {/* FIX 11: profile and partner are now defined above — no more undefined errors */}
      <DashboardHeader
